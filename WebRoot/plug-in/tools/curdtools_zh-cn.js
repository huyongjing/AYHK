...
try {
    ...
    if (windowapi && windowapi.data) {
        parentWin = windowapi.data.parentWin;
    }
} catch(e) {}
...
function maxIndex() {
    var dlgZindex = 2990;
    if (window.top && top.dlgZindex) {
        dlgZindex = top.dlgZindex;
    }
    top.dlgZindex = dlgZindex + 100;
    lhgdialog.setting.zIndex = top.dlgZindex;
    return dlgZindex;
}
 ...
 * @param addurl//目标页面地址
 */
function addkjjssq(title,addurl,gname,width,height) {
    gridname=gname;
    openwindow(title, addurl,"",width,height);
}
/**
 * 添加事件打开窗口--编辑科技计划申请用
 * @param title 编辑框标题
 * @param addurl//目标页面地址
 */
function updatekjjssq(title,url, id,width,height,isRestful) {
    gridname=id;
 ...
 * @param id//主键字段
 */
function batchUpdate(title,url, id,width,height,isRestful) {
    gridname=id;
    var ids = [];
...
function createdetailwindow(title, addurl,width,height) {
    ...
    // }
    if ((width+"").indexOf("%") > 0) {
        width = window.top.document.body.offsetWidth * (parseFloat(width) / 100) - 40;
    }
    if ((height+"").indexOf("%") > 0) {
        height =window.top.document.body.offsetHeight * (parseFloat(height) / 100) - 100;
    }
...
function tip(msg) {
    try{
        ...
        $.messager.show({
            ...
            timeout : 6000,
            style:{
                font: 'font: 20px/42px "微软雅黑",tahoma,arial,\5b8b\4f53,sans-serif',
                right:'',
                bottom:''
            }
        });
