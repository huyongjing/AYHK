    ...
    <div region="center" style="padding:0px;border:0px">
        <t:datagrid name="ayGzList" checkbox="true" fitColumns="false" title="工资表"
          ...
          <!--   <t:dgCol title="当月应出勤" field="dyycq" queryMode="single" hidden="true" width="120"  align="center" extendParams="editor:'numberbox'"  ></t:dgCol>
            ...
            <t:dgToolBar title="保存" icon="icon-add" url="ayGzController.do?saveRows" funname="saveData" width="800"></t:dgToolBar>
...
<script type="text/javascript">
    ...
    function changeEditGrid() {
        ...
        $.each(rows, function (i, row) {
            ...
            //通讯补助
            var txbz = $('#ayGzList').datagrid('getEditor', {index: i, field: 'txbz'});
            //出差补助
            var ccbz = $('#ayGzList').datagrid('getEditor', {index: i, field: 'ccbz'});
            //应发工资总额
            ...
            //银行帐号
            var yhzh = $('#ayGzList').datagrid('getEditor', {index: i, field: 'yhzh'});
             ...
             $(jxgz.target).attr("disabled", "disabled");
             $(ksjl.target).attr("disabled", "disabled");
             //$(txbz.target).attr("disabled", "disabled");
             //$(ccbz.target).attr("disabled", "disabled");
            ...
            var yczd = '<%=request.getAttribute("yczd")%>';
                if(yczd){
		    		if(yczd.indexOf(','>-1)){
		    			var columnArray = yczd.split(",");
		    			 $.each(columnArray, function (f, fuck) {
		    			  	var fuck_obj = $('#ayGzList').datagrid('getEditor', {index: i, field: fuck});
		    			 	$(fuck_obj.target).attr("value", "0");
		    			 });
            ...
            //工资计算
            jisuangongzi(jbgz,gwgz,zcgz,nzgz,jxgz,gljx,ksjl,qqj,qqkk,cfbz,jtbz,txbz,ccbz,zfbz,yfgzze,grcdhj,ynssde,mze,snssde,sl,sskc,grsds,shgz,qt,sfgz,geshui_obj,row);
    ...
    function jisuangongzi(jbgz,gwgz,zcgz,nzgz,jxgz,gljx,ksjl,qqj,qqkk,cfbz,jtbz,txbz,ccbz,zfbz,yfgzze,grcdhj,ynssde,mze,snssde,sl,sskc,grsds,shgz,qt,sfgz,geshui_obj,row){
    	//应发工资总额
            ...
            var v_txbz = parseFloat($(txbz.target).val())||0;
            var v_ccbz = parseFloat($(ccbz.target).val())||0;
            row.yfgzze = v_jbgz + v_gwgz + v_zcgz + v_nzgz + v_jxgz + v_gljx + v_ksjl + v_qqj + v_qqkk + v_cfbz + v_jtbz + v_zfbz + v_txbz+ v_ccbz;
            ...
            $(shgz.target).attr("value", row.shgz.toFixed(2));
            
            //实发工资 = 税后工资 + 其他
            var v_qt = parseFloat($(qt.target).val())||0;
            row.sfgz = row.shgz + v_qt;
            $(sfgz.target).attr("value", row.sfgz.toFixed(2));
    }
...
//导出
        function ExportXls() {
            JeecgExcelExport("ayGzController.do?exportXls", "ayGzList");
        }
