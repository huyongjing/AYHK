...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	...
	public void initAllAyKcpzEntity() {
		List<AyZxpzEntity> typeGroups = this.loadAll(AyZxpzEntity.class);
		if(typeGroups!=null&&typeGroups.size()>0){
			for (AyZxpzEntity tsTypegroup : typeGroups){
				String code = tsTypegroup.getZxbm();//中心编码
				String text = tsTypegroup.getZx();//中心名称
				ResourceUtil.allCenters.put(code,text);
			}
		}
	}
	/**
